...
// Encoder format — determines QKV layout and FSMN implementation
enum SenseVoiceEncoderFormat {
    FORMAT_UNKNOWN = 0,
    FORMAT_LOVEMEFAN = 1,   // separate QKV + CMVN + conv1d FSMN
    FORMAT_OFFICIAL = 2,    // fused QKV + raw fbank + shift-acc FSMN
    FORMAT_NANO = 3,        // FunASR-Nano: official encoder + adaptor
};
...
struct sense_voice_hparams {
    ...
    // Dual-format support
    SenseVoiceEncoderFormat encoder_format = FORMAT_LOVEMEFAN;
    float frontend_scale_factor = 1.0f;  // 1.0 for lovemefan, 32768.0 for official
};

// Format detection (implemented in format_detect.cpp)
