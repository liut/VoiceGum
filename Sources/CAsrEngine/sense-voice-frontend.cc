...
bool load_wav_file(const char *filename, int32_t *sampling_rate,
  ...
  auto data_bytes = header.subchunk2_size;
  if (data_bytes > kMaxWavDataBytes) data_bytes = kMaxWavDataBytes;

  *sampling_rate = header.sample_rate;
  auto speech_len = data_bytes / 2;
  if (speech_len == 0) return false;
