...
bool sense_voice_model_load(const char *path_model, sense_voice_context &sctx) {
    ...
    // Detect format before reading metadata
    auto fmt = detect_sensevoice_format(path_model);
    hparams.encoder_format = fmt;
    if (fmt == FORMAT_OFFICIAL || fmt == FORMAT_NANO)
        hparams.frontend_scale_factor = 32768.0f;
    SENSE_VOICE_LOG_INFO("%s: detected format=%d (scale=%.0f)\n", __func__, (int)fmt, hparams.frontend_scale_factor);
...

    {
        ...
        // load encoder weights — format-aware tensor mapping
        {
            bool is_official = (hparams.encoder_format == FORMAT_OFFICIAL || hparams.encoder_format == FORMAT_NANO);
            std::string enc_prefix = is_official ? "audio_encoder." : "encoder.";
            ...
            if (is_official) {
                set_sense_voice_encoder_layer_sanm_official(tmp_encoder0, sense_voice.tensors, 1, "encoders0");
                sense_voice.model->encoder->encoder0 = tmp_encoder0[0];
                set_sense_voice_encoder_layer_sanm_official(sense_voice.model->encoder->encoders_layer, sense_voice.tensors, hparams.n_encoder_layers - 1, "encoders");
                set_sense_voice_encoder_layer_sanm_official(sense_voice.model->encoder->tp_encoders_layer, sense_voice.tensors, hparams.n_tp_encoder_layers, "tp_encoders");
            } else {
            ...
            sense_voice.model->encoder->e_after_norm_w = sense_voice.tensors[enc_prefix + "after_norm.weight"];
            sense_voice.model->encoder->e_after_norm_b = sense_voice.tensors[enc_prefix + "after_norm.bias"];

            sense_voice.model->encoder->e_tp_norm_w = sense_voice.tensors[enc_prefix + "tp_norm.weight"];
            sense_voice.model->encoder->e_tp_norm_b = sense_voice.tensors[enc_prefix + "tp_norm.bias"];
